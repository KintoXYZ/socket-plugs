...
const dstChain = ChainSlug.KINTO_DEVNET;
// const srcChain = ChainSlug.KINTO_DEVNET;
...
export const main = async () => {
  ...
  try {
    ...
    if (senderPrivateKey) {
      ...
      const kintoSigner = getSignerFromChainSlug(ChainSlug.KINTO_DEVNET);
...
export const retry = async () => {
  const signer = getSignerFromChainSlug(ChainSlug.KINTO_DEVNET);
  const srcChain = ChainSlug.KINTO_DEVNET;
  const dstChain = ChainSlug.ARBITRUM_SEPOLIA;
